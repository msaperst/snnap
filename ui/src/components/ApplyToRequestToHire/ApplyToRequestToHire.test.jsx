...
describe('snnap form input', () => {
  ...
  beforeEach(() => {
    ...
    wrapper = Enzyme.mount(<ApplyToRequestToHire />);
  });

  it('is a drop down item', () => {
    const { container } = render(<ApplyToRequestToHire />);
  ...
  it('opens a modal when button is clicked', async () => {
    ...
    const modal = await waitFor(() =>
      screen.getByTestId('ApplyToRequestToHireModal')
    );
  ...
  it('opens the modal when button is clicked', () => {
    ...
    const button = wrapper.find('#openApplyToRequestToHireButton').at(0);
  ...
  it('validates values when submitted', () => {
    ...
    wrapper.find('#openApplyToRequestToHireButton').at(0).simulate('click');
