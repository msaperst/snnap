...
describe('profile page', () => {
  ...
  it('shows the password', async () => {
    ...
    expect(await password.findElement(By.css('h3')).getText()).toEqual('Password');
  });
  ...
  async function checkPasswordFields() {
    ...
    await driver.findElement(By.id('updatePasswordButton')).click();
    const danger = await driver.wait(until.elementLocated(By.className('alert-danger')));
  ...
  it('password error message goes away once success is had', async () => {
    const { danger, currentPassword, newPassword } = await checkPasswordFields();
    expect(await danger.getText()).toEqual('Password must be 6 or more characters');
