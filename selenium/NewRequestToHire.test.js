...
describe('new request to hire', () => {
  ...
  beforeEach(async () => {
    ...
    // login as a user
    await test.loginUser('newRequestToHireUser');
    await driver.get(Test.getApp());
    const gigs = driver.wait(until.elementLocated(By.id('gig-dropdown')));
    gigs.click();
    button = driver.wait(
      until.elementLocated(By.id('openNewRequestToHireButton'))
    );
    await button.click();
    modal = driver.wait(
      until.elementLocated(By.css('[data-testid="newRequestToHireModal"]'))
    );
  }, 10000);
